...
public class CreateOVHServersScript extends Script {
    ...
    private static final Logger log = LoggerFactory.getLogger(CreateOVHServersScript.class);
    ...
    public void createServer(Credential credential, ServiceProvider openstack, OVHServer server) throws BusinessException {
        ...
        if (server.getInstanceName() == null) {
            ...
            List<String> networksList = new ArrayList<>();
          	List<ServerNetwork> servNetworks = server.getNetwork();
          	for(ServerNetwork servNetwork : servNetworks)
                ...
                List<JsonObject> newServers = openstackAPI.computeAPI(urlServer, credential, null, "get", "server");
              	for (JsonObject newServerObj : newServers) {
                    ...
                    server.setOrganization(newServerObj.get("tenant_id").getAsString());
                  	if (newServerObj.get("OS-EXT-STS:task_state").getAsString().equalsIgnoreCase("spawning")) {
