...
public class ListScalewayVolumeTypes extends Script{
    
    private static final Logger logger = LoggerFactory.getLogger(ListScalewayVolumeTypes.class);
    ...
    public void execute(Map<String, Object> parameters) throws BusinessException {
        ...
        WebTarget target = client.target("https://"+SCALEWAY_URL+"/zones/"+zone_id+"/products/volumes");
        Response response = CredentialHelperService.setCredential(target.request(), credential).get();
        String value = response.readEntity(String.class);
        logger.info("response : " + value);
        logger.debug("response status : {}", response.getStatus());
        if (response.getStatus() < 300) {
            JsonObject volumeTypeObj = new JsonParser().parse(value).getAsJsonObject().get("volumes").getAsJsonObject();
